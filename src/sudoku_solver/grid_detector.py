...
# of the grid extent.  Outward must be generous: this Mask R-CNN under-segments
# the bottom edge on some datasets.  Measured over 90 held-out photos, 0.14
# solves 77 and 0.06 solves 62.
BAND_OUT = 0.14
BAND_IN = 0.04


def build_maskrcnn(num_classes: int = 2):
    """Mask R-CNN (ResNet-50 FPN) with ``num_classes`` heads, including background."""
...
class GridDetector:
    ...
    Pipeline:
      ...
      2. A quadrilateral is fitted to the segmentation mask itself, one line
         per side, so the corners follow the mask's actual perspective-skewed
         shape rather than any bounding box of it.
      3. Each of those four edges is then snapped onto the real grid border
         line found within a narrow band around it, correcting for the mask
         boundary being imprecise.  Candidate lines are restricted to that band
         and to matching orientation, so nothing outside the grid can win.
    """
    ...
    def _run(self, image: np.ndarray):
        ...
        corners = self._corners_from_mask(mask_np)
        if corners is None:
            contour = self._mask_contour(mask_np)
            if contour is None:
                raise RuntimeError("No contour found from detection mask.")
            corners = self._find_quad(contour)
        else:
            corners = self._refine_on_grid_lines(resized, mask_np, corners)
    ...
    @classmethod
    def _corners_from_mask(cls, mask: np.ndarray) -> np.ndarray | None:
        ...
        return cls._quad_from_lines(lines)

    @staticmethod
    def _quad_from_lines(lines: list[tuple]) -> np.ndarray | None:
    ...
    def _refine_on_grid_lines(
        ...
        this Mask R-CNN under-segments the bottom edge on some datasets, cutting
        ...
        refined = cls._quad_from_lines(lines)
        if refined is None:
            return quad
